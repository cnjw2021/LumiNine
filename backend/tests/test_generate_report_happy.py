...
def test_happy_flow_returns_pdf_bytes(monkeypatch):
    ...
    uc = GenerateReportUseCase(
        ...
        solar_starts_repo=MagicMock(spec=ISolarStartsRepository),
        solar_calendar_provider=MagicMock(spec=ISolarCalendarProvider),
        report_context_builder=MagicMock(spec=ReportContextBuilder),
        solar_terms_repo=MagicMock(spec=ISolarTermsRepository),
    )
